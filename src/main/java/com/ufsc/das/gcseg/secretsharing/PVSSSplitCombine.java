	...
	public Shares pVSSSplit(String secret) throws InvalidVSSScheme {
		...
		try {
			groupPrimeOrder = new BigInteger(
					Files.readAllLines(Path.of("./PVSSHelper/NumbersWith512Bits.txt")).get(lineNumber));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			g1 = new BigInteger(
					Files.readAllLines(Path.of("./PVSSHelper/NumbersWith511Bits.txt")).get(lineNumber));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			g2 = new BigInteger(
					Files.readAllLines(Path.of("./PVSSHelper/NumbersWith510Bits.txt")).get(lineNumber));
		} catch (Exception e) {
			e.printStackTrace();
		}
