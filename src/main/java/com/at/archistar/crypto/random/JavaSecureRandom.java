package com.at.archistar.crypto.random;
...
public class JavaSecureRandom implements RandomSource {
    ...
    private final ThreadLocal<SecureRandom> rng;
    ...
    public JavaSecureRandom() {
        try {
            rng = new ThreadLocal<>();
            rng.set(SecureRandom.getInstance(ALGORITHM));
        } catch (NoSuchAlgorithmException e) { // this should never happen
            throw new RuntimeException(e);
        }

        rng.get().nextBoolean(); // force the rng to seed itself
    }
